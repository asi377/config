...
class BullMQManager {
  ...
  createWorker(queueName, processor, options = {}) {
    // BullMQ workers use blocking Redis commands and require their own
    // connection with maxRetriesPerRequest: null (a hard BullMQ requirement,
    // see https://docs.bullmq.io/guide/going-to-production#maxretriesperrequest).
    const connection = redisClient.getConnection().duplicate({ maxRetriesPerRequest: null });
