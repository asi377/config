...
class NodeHealthService extends BaseService {
  async processHeartbeat({ nodeToken, serverId, metrics }) {
    ...
    const pending = await PendingNodeCommand.find({ serverId: server._id, status: 'pending' })
      .sort({ createdAt: 1 })
      .limit(20);

    if (pending.length) {
      await PendingNodeCommand.updateMany(
    ...
    return {
      registered: true,
      serverId: server._id,
      commands: pending.map(p => ({ id: p._id, type: p.type, ...p.params })),
    };
  }
