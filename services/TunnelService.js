...
class TunnelService {
        ...
        try {
            const tunnelConfig = await session.withTransaction(async () => {
                ...
                if (allocatedQuotaBytes !== null) {
                    const unallocatedBytes =
                        await this._getUnallocatedPoolBytes(sub._id, session);
                    if (allocatedQuotaBytes > unallocatedBytes) {
                        throw new InsufficientQuotaError();
                    }
                }

                const guestExpireDate =
                    isGuest && allocatedQuotaBytes === null
    ...
    async allocateServer() {
        ...
        if (!server) {
            throw new Error(
                'No available server — all servers are at full capacity',
            );
        }
